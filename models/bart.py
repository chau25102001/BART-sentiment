...
from models.modeling_outputs import (BaseModelOutput, BaseModelOutputWithPastAndCrossAttentions,
                                     CausalLMOutputWithCrossAttentions,
                                     Seq2SeqLMOutput,
                                     Seq2SeqModelOutput,
                                     Seq2SeqQuestionAnsweringModelOutput,
                                     Seq2SeqSequenceClassifierOutput, )
from models.config import BartConfig
...
class BartLearnedPositionalEmbedding(nn.Embedding):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int):
        self.offset = 2
        super(BartLearnedPositionalEmbedding, self).__init__(num_embeddings=num_embeddings + self.offset,
                                                             embedding_dim=embedding_dim)

    def forward(self, input_ids: torch.Tensor, past_key_values_length: int = 0):
        """`input_ids' shape is expected to be [bsz x seqlen]."""

        bsize, seq_len = input_ids.shape[:2]
        postions = torch.arange(past_key_values_length, past_key_values_length + seq_len, dtype=torch.long,
                                device=self.weight.device).expand(bsize, -1)
        return super().forward(postions + self.offset)
